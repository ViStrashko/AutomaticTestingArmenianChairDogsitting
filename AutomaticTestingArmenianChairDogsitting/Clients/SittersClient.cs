...
namespace AutomaticTestingArmenianChairDogsitting.Clients
{
    public class SittersClient
    {
        ...
        public void DeleteSitterByAdmin(int id, string token, HttpStatusCode expectedCode)
        {
            HttpClient client = new HttpClient();
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
            HttpRequestMessage message = new HttpRequestMessage()
            {
                Method = HttpMethod.Delete,
                RequestUri = new System.Uri($"{Urls.Sitters}/{id}/admin"),
            };
            HttpResponseMessage response = client.Send(message);
            HttpStatusCode actualCode = response.StatusCode;
            Assert.AreEqual(expectedCode, actualCode);
        ...
        public void UpdatePriceCatalog(PriceCatalogUpdateModel newPrices, string token, HttpStatusCode expectedCode)
        {
            ...
            {
                Method = HttpMethod.Patch,
                RequestUri = new System.Uri($"{Urls.Sitters}/prices"),
                Content= new StringContent(json, Encoding.UTF8, "application/json")
            };
            HttpResponseMessage response = client.Send(message);
            HttpStatusCode actualCode = response.StatusCode;
